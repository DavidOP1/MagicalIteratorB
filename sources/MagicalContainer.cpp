...
namespace ariel{
    ...
    void MagicalContainer::addElement(int temp){
        ...
        int position = this->getPosition(temp, this->elements);
        this->elements.insert(this->elements.begin()+position , num);
        //add element to prime vector only
        this->sideCrossPattern(num);
        if(this->checkPrime(temp)){
            position = this->getPosition(temp , this->prime);
     ...
     //////////////////////////////////////COMPARE VALS/////////////////////////////////////////
     bool MagicalContainer::compareVal(const int *val1 , const int *val2){
        return (*val1<*val2);
     }

     //////////////////////////////////MAKING A SIDE CROSS ARRAY////////////////////////////////
     void MagicalContainer::sideCrossPattern(int* num){
            sideCross.push_back(num);
            sort(sideCross.begin(), sideCross.end() , compareVal);
